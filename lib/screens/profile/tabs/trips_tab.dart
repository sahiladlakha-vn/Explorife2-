    ...
    return ListView(
      ...
      children: [
                      ...
                      Flexible(
                        child: Text(trip.displayName,
                            ...
                            style: GoogleFonts.bebasNeue(
                                fontSize: 24, color: _kInk)),
                      ),
                      const SizedBox(width: 4),
                      GestureDetector(
                        onTap: () => _openSwitcher(provider.trips, trip.id),
                        child: const Icon(Icons.expand_more,
                            size: 22, color: _kMute),
                      ),
                      const SizedBox(width: 4),
                      GestureDetector(
        ...
        switch (_segment) {
          ...
          TripDetailSegment.bookings => _BookingsSegment(tripId: trip.id),
        },
      ],
    );
  }

  void _openSwitcher(List<Trip> trips, String currentId) {
    showModalBottomSheet<void>(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (_) => _TripSwitcherSheet(
        trips: trips,
        currentId: currentId,
...
/// Bottom sheet listing every trip the user owns — tap one to switch My Trip
/// to it. Complements (doesn't replace) the drawer's full TripsListScreen:
/// this is for quick switching without leaving the tab.
class _TripSwitcherSheet extends StatelessWidget {
  final List<Trip> trips;
  final String currentId;
  final ValueChanged<String> onSelect;
  const _TripSwitcherSheet(
      {required this.trips, required this.currentId, required this.onSelect});
                  ...
                  return GestureDetector(
                    ...
                    child: Container(
                      ...
                      decoration: BoxDecoration(
                        ...
                        border: Border.all(
                            color: selected ? AppTheme.primary : _kBorder),
                      ),
                      child: Row(
                        children: [
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                ...
                                Text(
                                    Trip.formatDateRange(
                                        t.startDate, t.endDate),
                                    style: GoogleFonts.jetBrainsMono(
                                        fontSize: 11, color: _kMute)),
                              ],
                            ),
                          ),
                          if (selected)
...
class _TripSegmentState extends State<_TripSegment> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        const _CardHeader(
            icon: Icons.groups_outlined, label: 'TRAVELERS', color: AppTheme.primary),
        const SizedBox(height: 10),
        _TravelersCard(tripId: widget.trip.id, travelers: travelers),
        const SizedBox(height: 16),
        const _CardHeader(
            icon: Icons.description_outlined, label: 'DOCUMENTS', color: _kTeal),
        const SizedBox(height: 10),
        _DocumentsCard(
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 2),
                Text(roleLabel,
                    style: GoogleFonts.dmSans(fontSize: 11, color: _kMute)),
              ],
            ),
          ),
...
({String label, Color color})? _expiryAlertFor(TripDocument doc, Trip trip) {
  ...
  if (doc.type == DocumentType.passport) {
    final sixMoAfterEnd = DateTime(
        trip.endDate.year, trip.endDate.month + 6, trip.endDate.day);
            ...
            child: Column(
              children: [
                for (final (i, d) in documents.indexed)
                  _DocumentRow(
                      document: d, trip: trip, travelers: travelers, divider: i > 0),
              ],
            ),
          ),
        const SizedBox(height: 10),
            ...
            child: Stack(
              children: [
                ...
                FractionallySizedBox(
                    widthFactor: fill, child: Container(height: 6, color: _kGreen)),
              ],
            ),
          ),
          const SizedBox(height: 10),
...
class _PackingRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final assignee =
        assigneeId == null ? null : travelers.where((t) => t.id == assigneeId).firstOrNull;
                      ...
                      children: [
                        Expanded(
                          child: TextField(
                            ...
                            style: GoogleFonts.dmSans(fontSize: 14, color: _kInk),
                            decoration: _lightFieldDecoration('Category (optional)'),
                          ),
                        ),
                        const SizedBox(width: 12),
                        _QuantityStepper(
                        ...
                        children: [
                          ...
                          for (final t in widget.travelers)
                            _TypeChip(
                              ...
                              onTap: () =>
                                  setState(() => _assigneeCollaboratorId = t.id),
                            ),
                        ],
                      ),
                    ],
          ...
          children: [
            Text(_weekdayAbbr[date.weekday - 1],
                style: GoogleFonts.jetBrainsMono(
                    ...
                    fontWeight: FontWeight.w700,
                    letterSpacing: 0.6,
                    color: active ? Colors.white.withValues(alpha: 0.85) : _kMute)),
            const SizedBox(height: 2),
            Text('${date.day}',
                style: GoogleFonts.bebasNeue(
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: [
            Text('ADD',
                style: GoogleFonts.jetBrainsMono(
                    fontSize: 9, fontWeight: FontWeight.w700, color: _kMute, letterSpacing: 0.6)),
            const SizedBox(height: 2),
            Text('+', style: GoogleFonts.bebasNeue(fontSize: 20, color: _kMute)),
          ],
        ),
      ),
    );
            ...
            child: Row(
              children: [
                ...
                IconButton(
                  ...
                  icon: const Icon(Icons.copy_outlined, size: 18, color: _kMute),
                  splashRadius: 18,
                  constraints: const BoxConstraints(minWidth: 32, minHeight: 32),
                ),
                IconButton(
...
class _DaySummaryStrip extends StatelessWidget {
  ...
  const _DaySummaryStrip({required this.rollup});
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 4),
          Text('₫${Trip.formatVnd(rollup.plannedCostVnd, short: true)}',
              style: GoogleFonts.jetBrainsMono(fontSize: 11, color: _kMute)),
...
class _DayBody extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        if (stops.isNotEmpty) _DaySummaryStrip(rollup: rollup),
...
class _SlotSection extends StatelessWidget {
  ...
  Widget _rowFor(TripStop s, {int? dragHandleIndex}) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.stretch,
      children: [
        if (s.hasTransit) _TransitThread(stop: s),
        _SpotRow(
          stop: s,
...
IconData _transitIcon(String mode) {
  ...
  if (m.contains('train') || m.contains('rail')) return Icons.directions_railway;
...
class _TransitThread extends StatelessWidget {
  final TripStop stop;
  const _TransitThread({required this.stop});
  ...
  Widget build(BuildContext context) {
    ...
    if (stop.transitCostVnd != null) {
      parts.add('₫${Trip.formatVnd(stop.transitCostVnd!, short: true)}');
    }
    return Padding(
  ...
  Widget build(BuildContext context) {
    ...
    final metaParts = <String>[
      ...
      if (resolvedCost != null)
        (resolvedCost == 0
            ? 'Free'
            : '₫${Trip.formatVnd(resolvedCost, short: true)}'),
    ];
...
InputDecoration _lightFieldDecoration(String hint) => InputDecoration(
      hintText: hint,
      hintStyle: GoogleFonts.dmSans(fontSize: 14, color: _kMute),
      filled: true,
      fillColor: _kCard,
      contentPadding:
...
class _ActivityEditSheetState extends State<_ActivityEditSheet> {
  late final _titleCtrl = TextEditingController(text: widget.stop.customTitle ?? '');
  ...
  Widget build(BuildContext context) {
    ...
    // reliably occludes it instead, matching EditTripSheet/_TripSwitcherSheet.
    return DraggableScrollableSheet(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    TextField(
                      ...
                      style: GoogleFonts.dmSans(fontSize: 14, color: _kInk),
                      decoration: _lightFieldDecoration('Price — blank for TBD, 0 for free')
                          .copyWith(prefixText: '₫ '),
                    ),
...
// BOOKINGS SEGMENT
// ─────────────────────────────────────────
class _BookingsSegment extends StatelessWidget {
  final String tripId;
  const _BookingsSegment({required this.tripId});

  @override
  Widget build(BuildContext context) {
    final bookings = context.watch<BookingProvider>().bookingsFor(tripId);
    if (bookings.isEmpty) {
...
class _BookingCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = switch (booking.status) {
      BookingStatus.toBook => ('TO BOOK', _kMute),
  ...
  String _subtitle() {
    ...
    if (booking.startAt != null) {
      parts.add('${_months[booking.startAt!.month - 1]} ${booking.startAt!.day}');
    }
    return parts.join(' · ');
  }
}
...
class _DashboardSegmentState extends State<_DashboardSegment> {
  ...
  Widget build(BuildContext context) {
    ...
    final expenses = splits
        .expensesForTrip(trip.id)
    ...
    return Column(
      ...
      children: [
        const _CardHeader(
            icon: Icons.pie_chart_outline, label: 'OVERALL', color: AppTheme.primary),
        const SizedBox(height: 10),
        _OverallCard(spentVnd: spentVnd, budgetVnd: trip.budgetVnd),
        const SizedBox(height: 16),
        const _CardHeader(
            icon: Icons.bar_chart_outlined, label: 'PLANNED VS. ACTUAL', color: _kTeal),
        const SizedBox(height: 10),
        _PlannedVsActualCard(planned: planned, actual: actual),
        const SizedBox(height: 16),
        const _CardHeader(
            icon: Icons.receipt_long_outlined, label: 'EXPENSES', color: AppTheme.primary),
        const SizedBox(height: 10),
        _ExpensesCard(
          expenses: expenses,
        ...
        const SizedBox(height: 16),
        const _CardHeader(icon: Icons.swap_horiz, label: 'SETTLE UP', color: _kGreen),
        const SizedBox(height: 10),
        _SettleUpCard(balances: balances, travelerById: travelerById),
      ],
    );
  }
}
...
class _OverallCard extends StatelessWidget {
  final int spentVnd;
  final int budgetVnd;
  const _OverallCard({required this.spentVnd, required this.budgetVnd});
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              Text('₫${Trip.formatVnd(spentVnd, short: true)}',
                  style: GoogleFonts.bebasNeue(fontSize: 26, color: _kInk)),
              const SizedBox(width: 6),
              Text('/ ₫${Trip.formatVnd(budgetVnd, short: true)}',
                  style: GoogleFonts.dmSans(fontSize: 13, color: _kMute)),
              const Spacer(),
              Text('$pct% SPENT',
                  style: GoogleFonts.jetBrainsMono(
                      fontSize: 10.5, color: _kMute, fontWeight: FontWeight.w700)),
            ],
          ),
          const SizedBox(height: 10),
          ClipRRect(
...
class _PlannedVsActualCard extends StatelessWidget {
  ...
  const _PlannedVsActualCard({required this.planned, required this.actual});
...
class _CategoryBudgetRow extends StatelessWidget {
  ...
  const _CategoryBudgetRow(
      {required this.label, required this.plannedVnd, required this.actualVnd});
          ...
          children: [
            ...
            if (status.over) ...[
              ...
              Container(
                ...
                child: Text('OVER',
                    style: GoogleFonts.jetBrainsMono(
                        fontSize: 9, color: _kCritical, fontWeight: FontWeight.w700)),
              ),
            ],
            const Spacer(),
            Text(
                '₫${Trip.formatVnd(actualVnd, short: true)} / ₫${Trip.formatVnd(plannedVnd, short: true)}',
                style: GoogleFonts.jetBrainsMono(fontSize: 11, color: _kMute)),
          ],
        ),
        const SizedBox(height: 6),
...
class _ExpenseRow extends StatelessWidget {
  ...
  String get _metaLine {
    ...
    if (expense.expenseDate != null) parts.add(_fmtShortDate(expense.expenseDate!));
    return parts.join(' · ');
  }
          ...
          GestureDetector(
            ...
            child: Row(
              ...
              children: [
                ...
                const SizedBox(width: 8),
                Text('₫${Trip.formatVnd(expense.amount.round(), short: true)}',
                    style: GoogleFonts.dmSans(
                        fontSize: 14, fontWeight: FontWeight.w700, color: _kInk)),
                const SizedBox(width: 4),
...
class _SplitDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final equalShare = memberIds.isEmpty ? 0.0 : expense.amount / memberIds.length;
    final rows = hasRealShares
        ? shares
        : [
            for (final uid in memberIds)
              SplitExpenseShare(id: '$uid-equal', userId: uid, amount: equalShare),
          ];
    ...
    return Column(
      ...
      children: [
        Text(
            ...
            style: GoogleFonts.jetBrainsMono(
                fontSize: 9.5, color: _kMute, letterSpacing: 0.6, fontWeight: FontWeight.w700)),
...
class _SplitMemberRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final (label, color) = isPayer
        ...
        : (isSettled
            ? ('SETTLED', _kGreen)
            : ('OWES ₫${Trip.formatVnd(amountVnd, short: true)}', _kWarn));
    return Padding(
        ...
        children: [
          ...
          Expanded(
            child: Text(traveler?.displayName ?? 'Unknown',
                maxLines: 1,
                overflow: TextOverflow.ellipsis,
                style: GoogleFonts.dmSans(fontSize: 12.5, color: _kInk)),
          ),
          Container(
            padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
            decoration: BoxDecoration(
...
class _SettleUpCard extends StatelessWidget {
  ...
  final Map<String, TripTraveler> travelerById;
  const _SettleUpCard({required this.balances, required this.travelerById});
...
class _SettleUpRow extends StatelessWidget {
  ...
  final bool divider;
  const _SettleUpRow(
      {required this.balance, required this.from, required this.to, required this.divider});
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 10),
          Text('₫${Trip.formatVnd(balance.amountVnd.round(), short: true)}',
              style: GoogleFonts.jetBrainsMono(
