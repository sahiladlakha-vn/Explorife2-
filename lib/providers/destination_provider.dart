...
//
// Adds an explicit load lifecycle (initial → loading → ready / error) on top of
// the existing filtering API. Nothing the UI already calls changed name:
// destinations, featured, saved, findById, selectCategory, setSearchQuery,
// toggleSave, selectedCategory, searchQuery all behave as before.
//
// Data is still the in-memory seed for now, but it's loaded through an async
// load() so the UI can show real loading + error states. Swapping to Supabase
// is a one-method change — see the marked TODO in load().
...
class DestinationProvider extends ChangeNotifier {
  ...
  bool get isReady => _status == DestinationStatus.ready;
  ...
  /// Loads once. Safe to call on every build / from initState — it no-ops if a
  /// load has already started.
  Future<void> loadIfNeeded() async {
    if (_status == DestinationStatus.initial) {
      await load();
    }
  }

  /// (Re)loads the destination list.
  Future<void> load() async {
    ...
    try {
      // TODO(supabase): replace the seed + delay with a real query, e.g.
      //   final rows = await Supabase.instance.client
      //       .from('destinations').select();
      //   _all = rows.map<Destination>(Destination.fromMap).toList();
  ...
  // ── seed data (interim; replaced by Supabase in load()) ──
  List<Destination> _seed() => [
        Destination(
